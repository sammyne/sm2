...
func TestSqrt(t *testing.T) {
	...
	ns := randFieldVal(t)
	// the corresponding test in btcec is BUGGY without normalization
	if new(fieldVal).SqrtVal(&ns).Square().Normalize().Equals(&ns) {
...
func testSqrt(t *testing.T, test sqrtTest) {
	...
	default:
		...
		if new(fieldVal).SquareVal(&root).Equals(&f) ||
			new(fieldVal).SquareVal(&rootNeg).Equals(&f) {
			t.Fatalf("fieldVal.Sqrt root should not exist\n"+
				"got:     %v\ngot_neg: %v,\n%s\n", root, rootNeg, test.in)
		}
	}
}
